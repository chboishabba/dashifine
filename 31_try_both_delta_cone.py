...
def compute_deltas(
    ...
    if whiten != "none":
        ...
        for _, g in df.groupby(LABEL_COL):
            ...
            fwd = dA >= (-eps_arrow)
            if np.any(fwd):
                all_dX.append(dX[fwd])
        if all_dX:
            D = np.vstack(all_dX)
            if whiten == "diag":
                std = np.nanstd(D, axis=0)
                std = np.where(std < whiten_eps, 1.0, std)
                W = np.diag(1.0 / std)
            elif whiten == "full":
                C = np.cov(D, rowvar=False, bias=True)
                vals, vecs = np.linalg.eigh(C)
                vals = np.where(vals < whiten_eps, whiten_eps, vals)
...
def score_fixed_mask(
    ...
    pos_scale: float,
) -> Tuple[MethodResult, pd.DataFrame]:
    label_data = compute_deltas(df, x_cols, arrow_col, step_col, eps_arrow, whiten="none", whiten_eps=1e-12)
    n_steps_total = 0
    rows = []
    ...
    forward_frac_min = float(per_label["forward_frac"].min())
    forward_frac_mean = float(per_label["forward_frac"].mean())
    cone_frac_min = float(per_label["cone_forward_frac"].min())
    cone_frac_mean = float(per_label["cone_forward_frac"].mean())
    max_violation_max = float(per_label["max_Qd_violation"].max())
...
def dump_violations_csv(
    ...
    deltas = compute_deltas(df, x_cols, arrow_col, step_col, eps_arrow)
    rows = []
...
def run_method(
    ...
    label_data = compute_deltas(df, x_cols, arrow_col, step_col, eps_arrow, whiten, whiten_eps)
    # attach keep masks
    n_steps_total = 0
    for lab, d in label_data.items():
        keep = jump_filter_mask_per_label(d["dX"], d["forward"], jump_filter_frac)
...
def main() -> None:
    ...
    ap.add_argument("--freeze-mask", default="", help="Comma mask like '-1,1,-1' to score without search.")
    ap.add_argument("--freeze-pos-scale", type=float, default=None, help="If set, use this pos_scale for fixed mask.")
    ap.add_argument("--labels-include", default="", help="Regex to include labels (after parsing).")
    ap.add_argument("--labels-exclude", default="", help="Regex to exclude labels.")
    ap.add_argument("--per-label-out", default="", help="Write per-label scores for fixed mask.")
    ap.add_argument("--whiten", choices=["none","diag","full"], default="none",
                    help="Optional Δx whitening before cone tests (none|diag|full).")
    ap.add_argument("--whiten-eps", type=float, default=1e-12,
                    help="Eigenvalue/std floor for whitening.")
    args = ap.parse_args()
    ...
    if args.labels_include:
        pat = re.compile(args.labels_include)
        df = df[df[LABEL_COL].astype(str).apply(lambda s: bool(pat.search(s)))]
    if args.labels_exclude:
        pat = re.compile(args.labels_exclude)
        df = df[~df[LABEL_COL].astype(str).apply(lambda s: bool(pat.search(s)))]
    if df.empty:
    ...
    # If freeze mask is provided, skip search and just score the fixed mask.
    if args.freeze_mask.strip():
        mask = np.array([int(x) for x in args.freeze_mask.split(",")], dtype=int)
        pos_scale = float(args.freeze_pos_scale) if args.freeze_pos_scale is not None else 1.0
        res, per_label = score_fixed_mask(
            df=df,
            x_cols=x_cols,
            ...
            jump_filter_frac=args.jump_filter_frac,
            pos_scale=pos_scale,
        )
        print("\n=== Fixed mask score ===")
        print(pd.Series(res.__dict__).to_string())
        if args.per_label_out:
            per_label.to_csv(args.per_label_out, index=False)
            print(f"[ok] wrote: {args.per_label_out}")
        return
